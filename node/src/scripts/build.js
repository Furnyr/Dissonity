...
// Remove _unity_bridge
for (const path of entryPoints) {

  if (!path.includes("_unity_bridge")) continue;
...
if (!fs.existsSync("./src/client/nested/index.html") || !fs.existsSync("./src/client/nested/Build")) {
  throw "[ ⚠️ ] No Unity build found inside src/client/nested";
}

let htmlString = fs.readFileSync("./src/client/nested/index.html").toString();

// Canvas
htmlString = htmlString.replace("1920px", "100vw");
htmlString = htmlString.replace("1080px", "100vh");
htmlString = htmlString.replace("CCCCCC", "000000");
